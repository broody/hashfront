...
class GameThread:
    ...
    def __init__(self, game_id: int, tx_queue: TxQueue, only_player: int = 0):
        ...
        self._stale_override = {}      # {player_id: strategy_name} forced overrides
    ...
    def _tick(self, glog):
        ...
        actions = plan_turn(state, player, strategy_override=strat_override)
        ...
        strat_tag = f" [{strat_override}]" if strat_override else ""
        glog.info(f"R{state.info.round} P{player}: {len(my_units)}v{len(enemy_units)} — "
                  f"{n_moves}M {n_attacks}A {n_captures}C{strat_tag}")
